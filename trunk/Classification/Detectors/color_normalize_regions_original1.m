function patches_im=color_normalize_regions_original1(imgfile, featurefile,normalize_patch_size)
...
scaling=3; 
patch_counter=0;
patches_im=zeros((floor(normalize_patch_size/2)*2+1).^2,nb,size(img,3));
for f=1:nb
    patch_counter=patch_counter+1;
  angle=0;%estimate dominant orientation i.e., angle=dominant_orientation(patch);

  color_patch(:,:,:)=normalize_patch(img, feat(f,1),feat(f,2),feat(f,3),feat(f,4),feat(f,5),angle,scaling,normalize_patch_size);
%  color_patch(:,:,2)=normalize_patch(img(:,:,2), feat(f,1),feat(f,2),feat(f,3),feat(f,4),feat(f,5),angle,scaling,normalize_patch_size);
%  color_patch(:,:,3)=normalize_patch(img(:,:,3), feat(f,1),feat(f,2),feat(f,3),feat(f,4),feat(f,5),angle,scaling,normalize_patch_size);
  
%   figure(1);imagesc(img);axis image;
% figure(1);imshow(img);axis image;
  drawellipse(feat(f,1),feat(f,2), feat(f,3), feat(f,4),feat(f,5),scaling,'-y');
 
%     figure(2);imagesc(uint8(color_patch));axis image;
%  figure(2);imshow(uint8(color_patch),[]);axis image;

  patches_im(:,patch_counter,1:size(img,3))=reshape(color_patch,size(color_patch,1).^2,1,size(img,3));
%   pause;
...
imout=zeros(normalized_patch_size,normalized_patch_size,size(img,3));
% [h w]=size(img);
...
else imout(half_size+1+j,half_size+1+i,:)=zeros(1,size(img,3));
end
