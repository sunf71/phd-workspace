...
%                  waitbar(i/nimages,h);
i
%                    PHOG_result=Do_PHOG(sprintf('%s/%s',opts.imgpath,image_names{i}),assignment_opts.LBP_option,data_locations{i});
...
[minz index]=min(distance(double(points),vocabulary),[],2);
...
%%               %%%%%%%%%%%%% Local cordinate Coding (CVPR 2010) %%%%%%%%%%%%
              elseif(strcmp(assignment_opts.type,'LLC'))  
...
%                    All_hist(:,counter) = ScSPM_new(feature_struct,vocabulary, gamma, pyramid, weights, pooling);
                     All_hist(:,counter) =  LLC_pooling(feature_struct,vocabulary', pyramid, knn);
