...
% Parallelize into 4 processes

iterLen = length(mapkeys)
path = wrd.prefix;
parfor t=1:iterLen
  ...
  switch bk.segmentationType
      case 'quickshift'
          [Iseg map] = vl_quickseg(I_, bk.ratio, bk.sigma, bk.tau);
      case 'meanshift'
          [Iseg map] = msseg(I_, bk.hs, bk.hr, bk.M);
          map = map + 1;
      case 'graphbased'
          [Iseg map] = graphSeg(I_, bk.threshold, bk.min_size, ...
              bk.nRadius, bk.model);
      case 'normalizedcut'
          [Iseg map] = segmentMinCut(I_, bk.segmentsCount);
      otherwise
          UNDEFINED_VARIABLE(1) % Throw an exception
  end
  ...
  saveAll(file, Iseg, map, labels, elapsed, segs) ;
...
% To be able to call from parfor
function args = saveAll(file, Iseg, map, labels, elapsed, segs)
    save(file, 'Iseg', 'map', 'labels', 'elapsed', 'segs', '-MAT') ;
end
