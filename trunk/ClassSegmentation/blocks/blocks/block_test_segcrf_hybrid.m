function bk = block_test_segcrf_hybrid(bk, varargin)
...
for t=1:length(mapkeys)
  ...
  classRS = reshape(confidence, x * y, c)';
  [vals, inds] = maxk(classRS, 3);
  
  class1 = reshape(inds(1, :), x, y);
  class2 = reshape(inds(2, :), x, y);
  class3 = reshape(inds(3, :), x, y);
...
%   if (want_to_see)
%       close all;
%       I2 = bkfetch(bk.db.tag, 'image', seg_id);
%       
%       figure, imshow(I2);
%       figure, imshow(label2rgb(map));
...
%   end
  ...
  file = fullfile(path, bk.tag, [n '.mat']);
  saveClassConfidence(file, class1, class2, class3, confidence);
...
%   saveClassConfidence(file, class, confidence);
  ...
  saveNewLabelsSegDec(file, newlabels, segdec);
...
function saveClassConfidence(file, class1, class2, class3, confidence)
  save(file, 'class1', 'class2', 'class3', 'confidence', '-MAT');
end

function saveNewLabelsSegDec(file, newlabels, segdec)
  save(file, 'newlabels', 'segdec', '-MAT');
end
...
switch lower(what)
  ...
  case 'test'
    i = varargin{1} ;
    path = fullfile(bk.path, bk.tag, 'data', sprintf('%05d.mat', i)) ;
    data = load(path, '-MAT') ;
