function bk = block_train_crf_hybrid(bk, varargin)
...
for im = 1:length(images)
  ...
  for pathInd = 1:length(seg_res_paths)
    ...
    segs = [segs bkfetch(bk.qseg.tag, 'segs', sid)];
    map = [map bkfetch(bk.qseg.tag, 'segmap', sid)];
  end
  ...
  pairwises{im}{2} = sparse(boundarylen(double(map), length(segs)));
  clear pairwise;
...
switch lower(what)
  ...
  case 'params'
    path = fullfile(bk.path, bk.tag, 'params.mat') ;    
    data = load(path, '-MAT') ;
    varargout{1} = data.params ;
 
  case 'paramspace'
    path = fullfile(bk.path, bk.tag, 'paramspace.mat') ;    
    data = load(path, '-MAT') ;
