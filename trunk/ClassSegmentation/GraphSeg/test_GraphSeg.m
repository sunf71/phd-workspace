...
load clown;
IM = X;
IM = imread('/home/amounir/Workspace/DiffSegMethods/img_000070.jpg');

%smooth the image by coherence filter:
filted_I = CoherenceFilter(IM,struct('T',5,'rho',2,'Scheme','I', 'sigma', 1));
%adjacent neighborhood  model:
[Iseg L] = graphSeg(filted_I, 0.5, 50, 2, 0);
%k-nearest neighborhood model:
[Iseg Lnn] = graphSeg(filted_I, 0.5/sqrt(3), 50, 10, 1);
%display:
