...
% Create the results folder and cd to that folder
mkdir(wrd.prefix);
cd(wrd.prefix);
...
ex.db.path       = wrd.prefix;

if param.pascal_submission_test
...
ex.qseg.segmentationType = param.segmentationType;

% Quickshift segmentation parameters
ex.qseg.sigma = param.qseg_sigma;
ex.qseg.ratio = param.qseg_ratio; 
ex.qseg.tau   = param.qseg_tau;
ex.qseg.split = 16;
ex.qseg.path  = wrd.prefix;

% Meanshift segmentation parameters
ex.qseg.hs = 10;
ex.qseg.hr = 7;
ex.qseg.M = 30;

% Graphbased segmentation parameters
ex.qseg.threshold = 0.5;
ex.qseg.min_size = 50;
ex.qseg.nRadius = 2;
ex.qseg.model = 0;

% Normalized cut segmentation parameters
ex.qseg.segmentsCount = 100;
...
ex.feat.split      = 16 ;
ex.feat.path       = wrd.prefix;

ex.feat = bkplug(ex.feat, 'db', ex.db.tag) ;
ex.feat = bkplug(ex.feat, 'qseg', ex.qseg.tag);
...
if param.crf == 1
  ...
  ex.testcrf = block_test_segcrf_hybrid;
  ex.testcrf.restrict = param.crf_restrict;
  ex.testcrf.path     = wrd.prefix;
  ...
  ex.testcrf = bkplug(ex.testcrf, 'classifier', ex.classifier);
  ex.testcrf = bkplug(ex.testcrf, 'dict', ex.dict.tag);
...
if strcmp(param.db_type, 'pascal07') || ...
  (strcmp(param.db_type, 'pascal09') || ...
   strcmp(param.db_type, 'pascal2010') && ~param.pascal_submission_test)
  ex.vismulti = block_vismulti;
  ex.vismulti.tag = ['vismulti@' bkver(ex.testseg.tag)];
  ex.vismulti.path = wrd.prefix;
...
%% Create a pascal submission
if (strcmp(param.db_type, 'pascal09') || strcmp(param.db_type, 'pascal2010')) ...
        && param.pascal_submission_test
