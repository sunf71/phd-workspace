function [Iseg L] = graphSeg(orgImg, threshold, min_size, nRadius, model)
%Input:
%       img: the gray image
...
%All rights reserved.
[len wid dim] = size(orgImg);
orgImg = im2double(orgImg);
orgImg = orgImg / max(orgImg(:));
...
[Iseg L] = construct_Iseg(orgImg, L);
Iseg = reshape(Iseg, size(orgImg));
