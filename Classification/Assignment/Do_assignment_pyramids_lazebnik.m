function All_hist=Do_assignment_pyramids_lazebnik(opts,assignment_opts)
...
%     descriptors = getfield(points_out,'descriptor_points'); 
    ...
    %TMP  = load([data_locations{ii},'/',assignment_opts.descriptor_name]);  
    descriptors = getfield(load([data_locations{ii},'/',assignment_opts.descriptor_name]),'descriptor_points');
...
%             PointInRng=(points(:,1) > x_lo) & (points(:,1) <= x_hi) & ...
