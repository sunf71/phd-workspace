function All_hist_NegBB=Do_assignment_DetSW_NegBB(opts,assignment_opts)
%%%% for collecting negative examples for training set with object bounding
%%%% boxes of all the categories except the detected one
VOCINIT2007
...
%                     i
%                     display('This is the positive Class to be detected');
%                 else
                       i
...
%                       end
                      ...
                      for ijkl=1:length(rec.objects)
                          if(isequal(rec.objects(1,ijkl).class,opts.classes{assignment_opts.cls_num}))
                              display('Not for this category as it is positive at the moment');
                          else
