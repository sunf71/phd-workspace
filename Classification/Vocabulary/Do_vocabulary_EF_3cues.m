function []=Do_vocabulary_EF_3cues(opts,vocabulary_opts)
if nargin<2
    vocabulary_opts=[];
end
...
for i=1:nimages
     if(trainset(i)==1)
       %%%% for additional detectors and descriptors %%%%
...
%        
%        points_out=load([data_locations{i},'/',vocabulary_opts.descriptor_name2]);
%        points_det2 = getfield(points_out,'descriptor_points'); 
...
%        
%        points_out=load([data_locations{i},'/',vocabulary_opts.descriptor_name3]);
%        points_det3 = getfield(points_out,'descriptor_points'); 
...
if(vocabulary_opts.fusion_flag)
       for jj=1:length(vocabulary_opts.alpha)
           ...
           points_total=[(vocabulary_opts.alpha(jj))*points_total1,vocabulary_opts.alpha2(jj)*points_total2,vocabulary_opts.alpha3(jj)*points_total3];
...
%              Do_ikmeans(opts,vocabulary_opts,points_total)
    ...
    if(strcmp(vocabulary_opts.type,'Kmeans')) %%% conventional kmeans 
        ...
        elseif(strcmp(vocabulary_opts.type,'ikmeans')) %%% Integer kmeans (ECCV 2008)
            ...
            %%%% and assignment are done together 
            Do_ikmeans(opts,vocabulary_opts,points_total1)
