function [out2,corner,f_ang_y]=opponent_descriptor6b(patches_R,patches_G,patches_B,number_of_bins,smooth_flag,mirror_flag)
...
patch_in=zeros(patch_size,patch_size,3);
...
for(kk=1:number_of_patches)   
    ...
    patch_in(:,:,1)=reshape(patches_R(:,kk),patch_size,patch_size);
    patch_in(:,:,2)=reshape(patches_G(:,kk),patch_size,patch_size);
    patch_in(:,:,3)=reshape(patches_B(:,kk),patch_size,patch_size);
