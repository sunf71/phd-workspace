function descriptor_points=Do_ColorYUV(im,descriptor_name,image_dir,points,scale_magnif,detector_name,normalize_patch_size)
...
if isscalar(points)   %%%%%%%%% for Dense detection %%%%%%%%%
    ...
    %%%% Formula to incorporate CMYSpace
%     patches_R=1-patches_R;
%     patches_G=1-patches_G;
%     patches_B=1-patches_B;
    
            patches_R=(0.299.*patches_R)+(0.587.*patches_G)+(0.114.*patches_B);
        patches_G=(-0.147.*patches_R)-(0.289.*patches_G)+(0.436.*patches_B);
        patches_B=(0.615.*patches_R)-(0.515.*patches_G)-(0.100.*patches_B);
