function [svm_score,best_CV]=Do_Classifiy_libSVM_pascal2007_hkl(opts,classification_opts, rng)
...
%           histogram=[(0.4).*histogram1;(0.2).*histogram2;(0.05).*histogram3;(0.2).*histogram4;(1.5).*histogram5];
...
lambdas = 10.^[1:-.5:-8];
[outputs,model,accuracies]  = hkl(histogram_train',train_truth,lambdas,'logistic','hermite',[ .5 3 .1 4 ],...
	'Xtest',histogram_test','Ytest',test_truth,'maxactive',100,'memory_cache',1e9);
