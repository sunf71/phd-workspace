function [Mboost,E,V,S] = BoostMatrix(image,order,sigma)
...
% order:
%       is order of differentiation for standard color boosting the first
...
if(ischar(image))               % image is a file containing image names
	...
	E=M(1,1)+M(2,2)+M(3,3);	%compute total derivative energy
...
elseif (isfloat(image))         % image is a single color image
    ...
    elseif(order==1)
        ...
        R=reshape([  gDer(a(:,:,1),sigma,2,0), gDer(a(:,:,1),sigma,1,1), gDer(a(:,:,1),sigma,0,2)],1,3*size(a,1)*size(a,2)); 
        G=reshape([  gDer(a(:,:,2),sigma,2,0), gDer(a(:,:,1),sigma,1,1), gDer(a(:,:,2),sigma,0,2)],1,3*size(a,1)*size(a,2));
        B=reshape([  gDer(a(:,:,3),sigma,2,0), gDer(a(:,:,1),sigma,1,1), gDer(a(:,:,3),sigma,0,2)],1,3*size(a,1)*size(a,2));            
    end
    R = R - mean(R);
    G = G - mean(G);
    B = B - mean(B);    
